...
function ReclaimablesInArea(aiBrain, locType, range)
    ...
    local ents = SUtils.AIGetReclaimablesAroundLocationSorianEditClose(aiBrain, locType, range)
...
function EnemyHasLessThanUnitsWithCategory(aiBrain, numReq, category, bool)
    local testCat = category
    local enemyBrain = aiBrain:GetCurrentEnemy()
    local count = 0
    if not enemyBrain then
    ...
    local eUnits = aiBrain:GetUnitsAroundPoint(testCat, Vector(0,0,0), 100000, 'Enemy')
	if not bool and table.getn(eUnits) >= numReq then
		return true
	else
        if table.getn(eUnits) >= numReq then
            return false
		else
			return true
        end
