#****************************************************************************
#**
#**  File     :  /mods/Sorian Edit/lua/ai/aiattackutilities.lua
...
function GetBestThreatTarget(aiBrain, platoon, bSkipPathability)
    ...
    if platoon.MovementLayer == 'Water' then
        ...
        if aiBrain.SorianEdit then
            maxRange, selectedWeaponArc, turretPitch = GetNavalPlatoonMaxRangeSorianEdit(aiBrain, platoon)
        else
    ...
    for tIndex,threat in threatTable do
        ...
        if not bSkipPathability then
            if platoon.MovementLayer != 'Water' then
                ...
                local bestPos
                if aiBrain.SorianEdit then
                    bestPos = CheckNavalPathingSorianEdit(aiBrain, platoon, {threat[1], 0, threat[2]}, maxRange, selectedWeaponArc, turretPitch)
                else
...
function CheckNavalPathingSorianEdit(aiBrain, platoon, location, maxRange, selectedWeaponArc, turretPitch)
    local platoonUnits = platoon:GetPlatoonUnits()
    local platoonPosition = platoon:GetPlatoonPosition()
...
function SendPlatoonWithTransportsNoCheck(aiBrain, platoon, destination, bRequired, bSkipLastMove)
    ...
    if platoon.MovementLayer == 'Land' or platoon.MovementLayer == 'Amphibious' then
        ...
        #DUNCAN - try the land path nodefirst , not the transport marker as this will get units closer(thanks to SorianEdit).
        if not transportLocation then
...
function PlatoonGenerateSafePathTo(aiBrain, platoonLayer, start, destination, optThreatWeight, optMaxMarkerDist, testPathDist)
    ...
    --If we are within 100 units of the destination, don't bother pathing. (SorianEdit and Duncan AI)
    if (aiBrain.SorianEdit or aiBrain.Duncan) and (VDist2(start[1], start[3], destination[1], destination[3]) <= 100
    ...
    local startNode
    if (aiBrain.SorianEdit or aiBrain.Duncan) then
        startNode = GetClosestPathNodeInRadiusByLayerSorianEdit(location, destination, optMaxMarkerDist, platoonLayer)
    else
    ...
    local endNode
    if (aiBrain.SorianEdit or aiBrain.Duncan) then
        endNode = GetClosestPathNodeInRadiusByLayerSorianEdit(destination, destination, optMaxMarkerDist, platoonLayer)
    else
    ...
    local path
    if aiBrain.SorianEdit or aiBrain.Duncan then
        -- SorianEdit and Duncans AI are using a strong modified pathfinding with path shortcuts, range checks and path caching for better performance.
        path = GeneratePathSorianEdit(aiBrain, startNode, endNode, ThreatTable[platoonLayer], optThreatWeight, destination, location)
    else
...
function GeneratePathSorianEdit(aiBrain, startNode, endNode, threatType, threatWeight, destination, location)
    ...
    if (aiBrain.SorianEdit or aiBrain.Duncan) and VDist2Sq(location[1], location[3], startNode.position[1], startNode.position[3]) > 10000 and
    ...
    repeat
        ...
        for i, adjacentNode in lastNode.adjacent do
            ...
            if (aiBrain.SorianEdit or aiBrain.Duncan) and SUtils.DestinationBetweenPoints(destination, lastNode.position, newNode.position) then
...
#########################################################################
##Below this line are SorianEdit AI exclusive functions added for SorianEdit AI
#########################################################################


function GetNavalPlatoonMaxRangeSorianEdit(aiBrain, platoon)
    local maxRange = 0
    local selectedWeaponArc = 'none'
    local turretPitch = nil
    local platoonUnits = platoon:GetPlatoonUnits()
    ...
    #LOG('*AI DEBUG: GetNavalPlatoonMaxRangeSorianEdit maxRange: '..maxRange..' selectedWeaponArc: '..selectedWeaponArc..' turretPitch: '..turretPitch)
    return maxRange, selectedWeaponArc, turretPitch
end

function GetLandPlatoonMaxRangeSorianEdit(aiBrain, platoon)
    local maxRange = 0
    local selectedWeaponArc = 'none'
    local turretPitch = nil
    local platoonUnits = platoon:GetPlatoonUnits()
    ...
    #LOG('*AI DEBUG: GetLandPlatoonMaxRangeSorianEdit maxRange: '..maxRange..' selectedWeaponArc: '..selectedWeaponArc..' turretPitch: '..turretPitch)
    return maxRange, selectedWeaponArc, turretPitch
end

function GetClosestPathNodeInRadiusByLayerSorianEdit(location, destination, radius, layer)
...
function AIPlatoonSquadAttackVectorSorianEdit(aiBrain, platoon, bAggro)
...
#    end
    ...
    attackPos[3] != platoon.LastAttackDestination[oldPathSize][3] then
        ...
        if (not path and reason == 'NoPath') or bNeedTransports then
            usedTransports = SendPlatoonWithTransportsSorianEdit(aiBrain, platoon, attackPos, true, false, true)
        # Require transports over 512 away
        elseif VDist2Sq(position[1], position[3], attackPos[1], attackPos[3]) > 512*512 and inBase then
            usedTransports = SendPlatoonWithTransportsSorianEdit(aiBrain, platoon, attackPos, true, false, false)
        # use if possible at 256
        elseif VDist2Sq(position[1], position[3], attackPos[1], attackPos[3]) > 256*256 and inBase then
            usedTransports = SendPlatoonWithTransportsSorianEdit(aiBrain, platoon, attackPos, false, false, false)
        end
...
function AIPlatoonNavalAttackVectorSorianEdit(aiBrain, platoon)
...
function SendPlatoonWithTransportsSorianEdit(aiBrain, platoon, destination, bRequired, bSkipLastMove, waitLonger)
