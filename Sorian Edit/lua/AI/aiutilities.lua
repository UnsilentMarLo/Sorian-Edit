----------------------------------------------------------------
-- File     :  /mods/Sorian Edit/lua/ai/aiutilities.lua
...
function EngineerTryReclaimCaptureAreaSorianEdit(aiBrain, eng, pos)
    if not pos then
        return false
    end
...
function GetAssisteesSorianEdit(aiBrain, locationType, assisteeType, buildingCategory, assisteeCategory)
    if assisteeType == 'Factory' then
        -- Sift through the factories in the location
        local manager = aiBrain.BuilderManagers[locationType].FactoryManager
        return manager:GetFactoriesWantingAssistance(buildingCategory, assisteeCategory)
...
function GetBasePatrolPointsSorianEdit(aiBrain, location, radius, layer)
    if type(location) == 'string' then
        if aiBrain.HasPlatoonList then
            for k, v in aiBrain.PBM.Locations do
                if v.LocationType == location then
...
-- used by engineers to move to a safe location
function EngineerMoveWithSafePathSorianEdit(aiBrain, unit, destination)
    if not destination then
        return false
    end
    ...
    if not result or VDist2Sq(pos[1], pos[3], destination[1], destination[3]) > 65536 and unit.PlatoonHandle and not EntityCategoryContains(categories.COMMAND, unit) then
        ...
        -- Skip the last move... we want to return and do a build
        bUsedTransports = AIAttackUtils.SendPlatoonWithTransportsSorianEdit(aiBrain, unit.PlatoonHandle, destination, needTransports, true, needTransports)
...
function EngineerTryRepairSorianEdit(aiBrain, eng, whatToBuild, pos)
    if not pos then
        return false
    end
...
function AIFindPingTargetInRangeSorianEdit(aiBrain, platoon, squad, maxRange, atkPri, avoidbases)
    local position = platoon:GetPlatoonPosition()
    if not aiBrain or not position or not maxRange then
        return false
    end
    ...
    for x, z in AttackPositions do
        ...
        for _, v in atkPri do
            ...
            if retUnit and targetShields > 0 then
                ...
                local closestBlockingShield = AIBehaviors.GetClosestShieldProtectingTargetSorianEdit(unit, retUnit)
...
function AIFindAirAttackTargetInRangeSorianEdit(aiBrain, platoon, squad, atkPri, position)
...
-- We use both Blank Marker that are army names as well as the new Large Expansion Area to determine big expansion bases
function AIFindStartLocationNeedsEngineerSorianEdit(aiBrain, locationType, radius, tMin, tMax, tRings, tType, eng)
    local pos = aiBrain:PBMGetLocationCoords(locationType)
    if not pos then
        return false
    end
...
function AIFindBrainTargetInRangeSorianEdit(aiBrain, platoon, squad, maxRange, atkPri, avoidbases)
    local position = platoon:GetPlatoonPosition()
    if not aiBrain or not position or not maxRange then
        return false
    end
...
function AIFindUndefendedBrainTargetInRangeSorianEdit(aiBrain, platoon, squad, maxRange, atkPri)
    local position = platoon:GetPlatoonPosition()
    if not aiBrain or not position or not maxRange then
        return false
    end
...
function AIFindBrainNukeTargetInRangeSorianEdit(aiBrain, platoon, maxRange, atkPri, nukeCount, oldTarget)
...
function GetOwnUnitsAroundPointSorianEdit(aiBrain, category, location, radius, min, max, rings, tType, minRadius)
    local units = aiBrain:GetUnitsAroundPoint(category, location, radius, 'Ally')
    local index = aiBrain:GetArmyIndex()
...
function AIFindDefensiveAreaSorianEdit(aiBrain, unit, category, range, runShield)
    if not unit.Dead then
        -- Build a grid to find units near
        local gridSize = range / 5
        if gridSize > 150 then
...
function AIGetMarkerLocationsSorianEdit(aiBrain, markerType)
    local markerList = {}
    if aiBrain.TacticalBases then
        for k, v in aiBrain.TacticalBases do
            table.insert(markerList, {Position = v.Position, Name = k})
...
function AIFindDefensivePointNeedsStructureSorianEdit(aiBrain, locationType, radius, category, markerRadius, unitMax, tMin, tMax, tRings, tType)
    local pos = aiBrain:PBMGetLocationCoords(locationType)
    if not pos then
        return false
    end
...
function AIFindFirebaseLocationSorianEdit(aiBrain, locationType, radius, markerType, tMin, tMax, tRings, tType, maxUnits, unitCat, markerRadius)
    -- Get location of commander
    local estartX, estartZ = aiBrain:GetCurrentEnemy():GetArmyStartPos()
    local threatPos = {estartX, 0, estartZ}

    -- Get markers
    local markerList = AIGetMarkerLocationsSorianEdit(aiBrain, markerType)
