...
local SBC = import('/mods/Sorian Edit/lua/editor/SorianEditBuildConditions.lua')
...
function CommanderThreadImproved(cdr, platoon)
    local aiBrain = cdr:GetAIBrain()
    aiBrain:BuildScoutLocations()
    -- Added to ensure we know the start locations (thanks to SorianEdit).
    SetCDRHome(cdr, platoon)
...
function AirLandToggleSorianEdit(platoon)
    for _, v in platoon:GetPlatoonUnits() do
        if not v.Dead and not v.AirLandToggleThreadSorianEdit then
            v.AirLandToggleThreadSorianEdit = v:ForkThread(AirLandToggleThreadSorianEdit)
        end
    end
end

function AirLandToggleThreadSorianEdit(unit)
...
local SurfacePrioritiesSorianEdit = {
    'EXPERIMENTAL ENERGYPRODUCTION STRUCTURE',
...
local T4WeaponPrioritiesSorianEdit = {
    'COMMAND',
    'EXPERIMENTAL ENERGYPRODUCTION STRUCTURE',
    'EXPERIMENTAL STRATEGIC STRUCTURE',
    'EXPERIMENTAL ARTILLERY OVERLAYINDIRECTFIRE',
...
function CDRRunAwaySorianEdit(aiBrain, cdr)
    local shieldPercent
    local cdrPos = cdr:GetPosition()
    ...
    if cdr:GetHealthPercent() < 0.7 or shieldPercent < 0.3 or nmeHardcore > 0 or nmeT3 > 4 then
        ...
        if nmeAir > 4 or nmeLand > 9 or nmeT3 > 4 or nmeHardcore > 0 or cdr:GetHealthPercent() < 0.7 or shieldPercent < 0.3 then
            ...
            repeat
                ...
                if canTeleport then
                    runSpot = AIUtils.AIFindDefensiveAreaSorianEdit(aiBrain, cdr, category, 10000, runShield)
                else
                    runSpot = AIUtils.AIFindDefensiveAreaSorianEdit(aiBrain, cdr, category, 100, runShield)
                end
...
function CDROverChargeSorianEdit(aiBrain, cdr)
    local weapBPs = cdr:GetBlueprint().Weapon
    local weapon
    for k, v in weapBPs do
        if v.Label == 'OverCharge' then
...
function CDRReturnHomeSorianEdit(aiBrain, cdr, Mult)
    -- This is a reference... so it will autoupdate
    local cdrPos = cdr:GetPosition()
    local rad = 100 * Mult
...
function CommanderBehaviorSorianEdit(platoon)
    for _, v in platoon:GetPlatoonUnits() do
        if not v.Dead and not v.CommanderThread then
            v.CommanderThread = v:ForkThread(CommanderThreadSorianEdit, platoon)
        end
    end
end
...
function CDRHideBehavior(aiBrain, cdr)
    if cdr:IsIdleState() then
        ...
        if category then
            runPos = AIUtils.AIFindDefensiveAreaSorianEdit(aiBrain, cdr, category, 100, runShield)
            IssueClearCommands({cdr})
...
function CommanderThreadSorianEdit(cdr, platoon)
    ...
    while not cdr.Dead do
        ...
        UCBC.HaveGreaterThanUnitsWithCategory(aiBrain,  1, 'FACTORY') and aiBrain:GetNoRushTicks() <= 0 then
            CDROverChargeSorianEdit(aiBrain, cdr)
        end
        WaitTicks(1)

        -- Run Away
        if not cdr.Dead then CDRRunAwaySorianEdit(aiBrain, cdr) end
        WaitTicks(1)

        -- Go back to base
        if not cdr.Dead then CDRReturnHomeSorianEdit(aiBrain, cdr, Mult) end
        WaitTicks(1)
        ...
        and not cdr:IsUnitState("Enhancing") and not (SUtils.XZDistanceTwoVectorsSq(cdr.CDRHome, cdr:GetPosition()) > 100) then
            ...
            elseif cdr.EngineerBuildQueue and table.getn(cdr.EngineerBuildQueue) ~= 0 then
                if not cdr.NotBuildingThread then
                    cdr.NotBuildingThread = cdr:ForkThread(platoon.WatchForNotBuildingSorianEdit)
                end
            end
        end
        WaitSeconds(1)
...
function AirUnitRefitSorianEdit(self)
    for k, v in self:GetPlatoonUnits() do
        if not v.Dead and not v.RefitThread then
            v.RefitThread = v:ForkThread(AirUnitRefitThreadSorianEdit, self:GetPlan(), self.PlatoonData)
        end
    end
end

function AirUnitRefitThreadSorianEdit(unit, plan, data)
    unit.PlanName = plan
    if data then
        unit.PlatoonData = data
    end
            ...
            if aiBrain:GetCurrentUnits(categories.AIRSTAGINGPLATFORM - categories.CARRIER - categories.EXPERIMENTAL) > 0 then
                ...
                if table.getn(plats) > 0 then
                    ...
                    if closest then
                        ...
                        IssueTransportLoad({unit}, closest)
                        if EntityCategoryContains(categories.AIRSTAGINGPLATFORM, closest) and not closest.AirStaging then
                            closest.AirStaging = closest:ForkThread(AirStagingThreadSorianEdit)
                            closest.Refueling = {}
                        elseif EntityCategoryContains(categories.CARRIER, closest) and not closest.CarrierStaging then
                            closest.CarrierStaging = closest:ForkThread(CarrierStagingThread)
                            closest.Refueling = {}
...
function AirStagingThreadSorianEdit(unit)
    local aiBrain = unit:GetAIBrain()
    while not unit.Dead do
        local ready = true
        local numUnits = 0
...
AssignExperimentalPrioritiesSorianEdit = function(platoon)
    local platoonUnits = platoon:GetPlatoonUnits()
    for k, v in platoonUnits do
        if v and not v.Dead then
            SetLandTargetPrioritiesSorianEdit(v, T4WeaponPrioritiesSorianEdit)
        end
    end
end

SetLandTargetPrioritiesSorianEdit = function(self, priTable)
...
WreckBaseSorianEdit = function(self, base)
    for _, priority in SurfacePrioritiesSorianEdit do
        local numUnitsAtBase = 0
        local notDeadUnit = false
...
FindExperimentalTargetSorianEdit = function(self)
    local aiBrain = self:GetBrain()
    if not aiBrain.InterestList or not aiBrain.InterestList.HighPriority then
        -- No target
        return
...
CommanderOverrideCheckSorianEdit = function(self)
...
GetHighestThreatClusterLocationSorianEdit = function(aiBrain, experimental)
...
CzarBehaviorSorianEdit = function(self)
    local aiBrain = self:GetBrain()
    if not aiBrain:PlatoonExists(self) then
        return
    end

    if not self:GatherUnitsSorianEdit() then
        return
    end

    AssignExperimentalPrioritiesSorianEdit(self)

    local cmd
    local targetUnit, targetBase = FindExperimentalTargetSorianEdit(self)
    local platoonUnits = self:GetPlatoonUnits()
    local oldTargetUnit = nil
    while aiBrain:PlatoonExists(self) do
        self:MergeWithNearbyPlatoonsSorianEdit('ExperimentalAIHubSorianEdit', 50, true)
        ...
        local nearCommander = CommanderOverrideCheckSorianEdit(self)
        local oldCommander = nil
        while nearCommander and aiBrain:PlatoonExists(self) and self:IsCommandsActive(cmd) do
            ...
            oldCommander = nearCommander
            nearCommander = CommanderOverrideCheckSorianEdit(self)
        end
        WaitSeconds(1)
...
AhwassaBehaviorSorianEdit = function(self)
    local aiBrain = self:GetBrain()
    if not aiBrain:PlatoonExists(self) then
        return
    end
    ...
    AssignExperimentalPrioritiesSorianEdit(self)
    local targetLocation = GetHighestThreatClusterLocationSorianEdit(aiBrain, self)
    local oldTargetLocation = nil
    local platoonUnits = self:GetPlatoonUnits()
    while aiBrain:PlatoonExists(self) do
        self:MergeWithNearbyPlatoonsSorianEdit('ExperimentalAIHubSorianEdit', 50, true)
        ...
        oldTargetLocation = targetLocation
        targetLocation = GetHighestThreatClusterLocationSorianEdit(aiBrain, self)
    end
end

TickBehaviorSorianEdit = function(self)
    local aiBrain = self:GetBrain()
    if not aiBrain:PlatoonExists(self) then
        return
    end
...
function ScathisBehaviorSorianEdit(self)
    local aiBrain = self:GetBrain()
    ...
    while aiBrain:PlatoonExists(self) do
        if lastBase then
            targetUnit, lastBase = WreckBaseSorianEdit(self, lastBase)
        end

        if not lastBase then
            targetUnit, lastBase = FindExperimentalTargetSorianEdit(self)
        end
        ...
        while aiBrain:PlatoonExists(self) and targetUnit and not targetUnit.Dead do
            ...
            if not airUnit then
                closestBlockingShield = GetClosestShieldProtectingTargetSorianEdit(experimental, experimental)
            end
            closestBlockingShield = closestBlockingShield or GetClosestShieldProtectingTargetSorianEdit(experimental, targetUnit)
...
function FatBoyBehaviorSorianEdit(self)
    if not self:GatherUnitsSorianEdit() then
        return
    end
    ...
    while aiBrain:PlatoonExists(self) do
        ...
        while aiBrain:PlatoonExists(self) and targetUnit and not targetUnit.Dead and useMove == InWaterCheck(self) and self:IsCommandsActive(cmd) do
            ...
            while closestBlockingShield do
                ...
                closestBlockingShield = closestBlockingShield or GetClosestShieldProtectingTargetSorianEdit(experimental, oldTarget)
                WaitSeconds(1)
            end
            WaitSeconds(1)
        end
...
function BehemothBehaviorSorianEdit(self)
    if not self:GatherUnitsSorianEdit() then
        return
    end
    AssignExperimentalPrioritiesSorianEdit(self)
...
function GetClosestShieldProtectingTargetSorianEdit(attackingUnit, targetUnit)
