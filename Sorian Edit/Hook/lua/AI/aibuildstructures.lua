...
-- function AIExecuteBuildStructure(aiBrain, builder, buildingType, closeToBuilder, relative, buildingTemplate, baseTemplate, reference, NearMarkerType)
	...
	-- -- end
	-- if 1 == 1 then
		-- LOG('*------------------------------- Sorian Edit AI called old AIExecuteBuildStructure:', repr(aiBrain.Name)) -- never uncomment will crash game
	-- end
...
function AIExecuteBuildStructureEdit(aiBrain, builder, buildingType, closeToBuilder, relative, buildingTemplate, baseTemplate, reference, NearMarkerType, cons)
	...
	-- end
	-- if 1 == 1 then
		-- LOG('*------------------------------- Sorian Edit AI called new AIExecuteBuildStructure:', repr(aiBrain.Name)) -- never uncomment will crash game
	-- end
    local factionIndex = aiBrain:GetFactionIndex()
    local whatToBuild = aiBrain:DecideWhatToBuild(builder, buildingType, buildingTemplate)
    local FactionIndexToName = {[1] = 'UEF', [2] = 'AEON', [3] = 'CYBRAN', [4] = 'SERAPHIM', [5] = 'NOMADS' }
    ...
    -- if we want to build a factory use the Seraphim Awassa for a bigger build place or if its early game, build the opener factory near a mex
    if buildingType == 'T1LandFactory' or buildingType == 'T2LandFactory' or buildingType == 'T3LandFactory' or buildingType == 'T1AirFactory' or buildingType == 'T2AirFactory' or buildingType == 'T3AirFactory' then
    ...
    elseif buildingType == 'T1SeaFactory' or buildingType == 'T2SeaFactory' or buildingType == 'T3SeaFactory' then
        buildingTypeReplace = 'T4SeaExperimental1'
        whatToBuildReplace = 'uas0401'
    end
