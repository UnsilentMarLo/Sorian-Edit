...
-- function PlatoonGenerateSafePathToSorianEdit(aiBrain, platoonLayer, startPos, endPos, optThreatWeight, optMaxMarkerDist, testPathDist)
    ...
    -- local path = GeneratePathUveso(aiBrain, startNode, endNode, ThreatTable[platoonLayer], optThreatWeight, endPos, startPos) -- GeneratePathSorianEdit errors on settime being nil
...
-- function EngineerGenerateSafePathToSorianEdit(aiBrain, platoonLayer, startPos, endPos, optThreatWeight, optMaxMarkerDist)
    ...
    -- --Generate the safest path between the start and destination
    -- local path = EngineerGeneratePathSorianEdit(aiBrain, startNode, endNode, ThreatTable[platoonLayer], optThreatWeight, endPos, startPos)
    -- if not path then return false, 'NoPath' end
...
-- function EngineerGeneratePathSorianEdit(aiBrain, startNode, endNode, threatType, threatWeight, endPos, startPos)
    -- threatWeight = threatWeight or 1
    -- -- Check if we have this path already cached.
    -- if aiBrain.PathCache[startNode.name][endNode.name][threatWeight].path then
        -- -- Path is not older then 30 seconds. Is it a bad path? (the path is too dangerous)
...
-- function CanGraphAreaTo(startPos, destPos, layer)
    ...
    -- if graphTable then
        ...
        -- end
    -- end
    -- --WARN('* AI-SorianEdit: CanGraphAreaTo: Start Area: '..repr(Scenario.MasterChain._MASTERCHAIN_.Markers[startNode.name].GraphArea)..' - End Area: '..repr(Scenario.MasterChain._MASTERCHAIN_.Markers[endNode.name].GraphArea)..'')
