...
function ExecutePlan(aiBrain)
    ...
    if aiBrain.SorianEdit then
        aiBrain:ForkThread(EcoManagerThreadSorianEdit, aiBrain)
    end

    return SorianEditExecutePlanFunction(aiBrain)
...
function EcoManagerThreadSorianEdit(aiBrain)
    ...
    --LOG('* AI-SorianEdit: Function EcoManagerThreadSorianEdit() started! CheatFactor:('..repr(CheatMultOption)..') - BuildFactor:('..repr(BuildMultOption)..') ['..aiBrain.Nickname..']')
    local Engineers = {}
    local paragons = {}
    local Factories = {}
    local lastCall = 0
    ...
    while aiBrain.Result ~= "defeat" do
        --LOG('* AI-SorianEdit: Function EcoManagerThreadSorianEdit() beat. ['..aiBrain.Nickname..']')
        coroutine.yield(5)
        ...
        -- Cheatbuffs
        if personality == 'SorianEditx' then
            ...
            if (GetGameTimeSeconds() > 60 * 1) and lastCall+10 < GetGameTimeSeconds() then
                ...
                if GetGameTimeSeconds() > 60 * 50 then
                    ...
                    SetArmyPoolBuffSorianEdit(aiBrain, CheatMult, BuildMult)
...
-- ECO for STRUCTURES
        if aiBrain:GetEconomyStoredRatio('ENERGY') < 0.01 and not aiBrain.HasParagon then
            ...
            if aiBrain:GetEconomyStoredRatio('ENERGY') < 0.01 then
                -- Disable Nuke
                if DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.MASSFABRICATION, 'MassFab') then busy = true
                -- Disable AntiNuke
                elseif DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.NUKE * (categories.TECH3 + categories.EXPERIMENTAL), 'Nuke') then busy = true
                -- Disable Massfabricators
                elseif DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.ANTIMISSILE * categories.SILO * categories.TECH3, 'AntiNuke') then busy = true
                -- Disable Intel
                elseif DisableUnitsSorianEdit(aiBrain, categories.RADAR + categories.OMNI + categories.SONAR, 'Intel') then busy = true
                -- Disable ExperimentalShields
                elseif DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.SHIELD * categories.EXPERIMENTAL, 'ExperimentalShields') then busy = true
                -- Disable NormalShields
                elseif DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.SHIELD - categories.EXPERIMENTAL, 'NormalShields') then busy = true
                end
        ...
        if aiBrain:GetEconomyStoredRatio('MASS') < 0.0 and not aiBrain.HasParagon then
            ...
            if aiBrain:GetEconomyStoredRatio('MASS') < 0.0 then
                -- Disable AntiNuke
                if DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.ANTIMISSILE * categories.SILO * categories.TECH3, 'AntiNuke') then busy = true
                end
            elseif aiBrain:GetEconomyStoredRatio('MASS') < 0.01 then
                -- Disable Nuke
                if DisableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.NUKE * (categories.TECH3 + categories.EXPERIMENTAL), 'Nuke') then busy = true
                end
            end
        elseif aiBrain:GetEconomyStoredRatio('ENERGY') > 0.50 then
            if aiBrain:GetEconomyStoredRatio('MASS') > 0.01 or aiBrain.HasParagon then
                ...
                if EnableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.SHIELD - categories.EXPERIMENTAL, 'NormalShields') then busy = true
                -- Enable ExperimentalShields
                elseif EnableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.SHIELD * categories.EXPERIMENTAL, 'ExperimentalShields') then busy = true
                -- Enable Intel
                elseif EnableUnitsSorianEdit(aiBrain, categories.RADAR + categories.OMNI + categories.SONAR, 'Intel') then busy = true
                -- Enable AntiNuke
                elseif EnableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.ANTIMISSILE * categories.SILO * categories.TECH3, 'AntiNuke') then busy = true
                -- Enable massfabricators
                elseif EnableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.MASSFABRICATION, 'MassFab') then busy = true
                -- Enable Nuke
                elseif EnableUnitsSorianEdit(aiBrain, categories.STRUCTURE * categories.NUKE * (categories.TECH3 + categories.EXPERIMENTAL), 'Nuke') then busy = true
                end
...
function BaseTargetManagerThreadSorianEdit(aiBrain)
    ...
    LOG('* AI-SorianEdit: Function BaseTargetManagerThread() started. ['..aiBrain.Nickname..']')
    ...
    while aiBrain.Result ~= "defeat" do
        --LOG('* AI-SorianEdit: Function BaseTargetManagerThread() beat. ['..aiBrain.Nickname..']')
        ClosestTarget = nil
        distance = 8192
        coroutine.yield(50)
...
function MarkerGridThreatManagerThreadSorianEdit(aiBrain)
    ...
    LOG('* AI-SorianEdit: Function MarkerGridThreatManagerThread() started. ['..aiBrain.Nickname..']')
    ...
    if not (PathGraphs['Land'] or PathGraphs['Amphibious'] or PathGraphs['Air'] or PathGraphs['Water']) then
        WARN('* AI-SorianEdit: Function MarkerGridThreatManagerThread() No AI path markers found on map. Threat handling diabled!  '..ScenarioInfo.ArmySetup[aiBrain.Name].AIPersonality)
        -- end this forked thread
        return
    end
    while aiBrain.Result ~= "defeat" do
        HighestThreat[armyIndex] = HighestThreat[armyIndex] or {}
        HighestThreat[armyIndex].ThreatCount = 0
        --LOG('* AI-SorianEdit: Function MarkerGridThreatManagerThread() beat. ['..aiBrain.Nickname..']')
